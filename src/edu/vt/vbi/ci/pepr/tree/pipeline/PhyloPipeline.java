...
public class PhyloPipeline {
	...
	public static void main(String[] args) {
		...
		remainderSeconds = elapsedSeconds%60;
	...
	public PhyloPipeline(String[] args) {
		...
		//set runName
		runName = "PhyloPipeline_" + System.currentTimeMillis();
		...
		Logger logger = Logger.getLogger("PhyloPipeline");
	...
	private void refineTree(String initialTree, CommandLineProperties clp,
		...
		System.out.println(">PhyloPipeline2.refineTree()");
	...
	private FastaSequenceFile[] filterOutDuplicateGenera(
		...
		for(int i = 0; i < homologySearchSequenceFiles.length; i++) {
			String taxon = homologySearchSequenceFiles[i].getTaxa()[0];
			System.out.println("checking taxon: " + taxon);
			String[] taxonTokens = spacePat.split(taxon);
			if(taxonTokens.length > 1) {
				String genus = taxonTokens[0];
				if(!generaKept.containsKey(genus)) {
					System.out.println("new genus: " + genus + "\t" 
							+ taxon);
					keepFiles.add(homologySearchSequenceFiles[i]);
					generaKept.put(genus,homologySearchSequenceFiles[i]);
				}
				else {
					System.out.println("duplicate genus: " + genus + 
							"\t" + taxon);
					FastaSequenceFile previouSpeciesFile = 
					...
					int currentGeneCount = homologySearchSequenceFiles[i].getSequenceCount();
					if(currentGeneCount > previousGeneCount) {
						System.out.println("replacing genus representative for "
								+ genus + " with " + taxon + ". " +
		...
		System.out.println("genomes before filtering: " + homologySearchSequenceFiles.length);
		System.out.println("genomes after filtering: " + r.length);
		return r;
	}
	...
	private String enhanceHomologGroups(String hgDirName,
		...
		System.out.println(">PhyloPipeline.enhanceHomologGroups()");
		String r = hgDirName;
	...
	public static void setCommandPaths() {
		...
		System.out.println("bin directory: " + binDirName);
	...
	private TextFile filterForBidirectional(TextFile hitPairFile, int scoreField) throws IOException {
		...
		if(verbose > 0) {
			System.out.println("filterForBidirectional() lines in input file: "
					+ lineCount);
		}
...
//		System.out.println("sorted");
		...
		if(verbose > 0) {
			System.out.println("lines in filtered file: " + r.getLineCount());
		}
		return r;
	}
	...
	private TextFile filterHitPairFile(TextFile hitPairFile) throws IOException {
		TextFile r = null;
		if(verbose > 0) {
			System.out.println("PhyloPipeline.filterHitPairFile() " 
		...
		if(verbose > 0) {
			System.out.println("Done filtering for columns. " +
...
//					"1", 
	...
	private void checkForRequiredPrograms() {
		...
		for(int i = 0; i < programFound.length; i++) {
			...
			if(programFound[i]) {
				logger.info("required program '" + requiredPrograms[i] + 
						"' found at: " + path);
			} else {
				logger.info("required program '" + requiredPrograms[i] +
						"' was not found");
		...
		for(int i = 0; i < requiredPrograms.length; i++) {
			System.out.println("required program found: " + 
